...
| 鼠标 / 手柄 | 鼠标可用；手柄**未测试** |
| 存档导入导出 | 走「下载」目录往返，但**只在部分机型上可用**（手机 Mate 80 Pro 上该权限申请不到）—— 见[已知限制](docs/LIMITATIONS.md) |
| 模组 | 可用。三个入口：游戏自带的「导入模组」按钮、悬浮球菜单的「导入模组」、或把文件放进 `Download/MindustryMods/`（⚠️ 最后一个**只在部分机型上可用**）。后两个**需要重启生效** —— 见[已知限制](docs/LIMITATIONS.md) |
