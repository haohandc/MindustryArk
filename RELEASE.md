...
### ⚠️ 一次**tag 指错**的事故记录（2026-09-21，**已修并核实**）
...
**已执行的修复与核实结果**（2026-09-21）：

| 项 | 修前 | 修后 |
|---|---|---|
| `refs/tags/v0.1.0-beta.1` | `bba382e…` ❌ | **`332192e…`** ✅ |
| release id | `392362924` | **未变** ✅ |
| 标题 / prerelease / draft | —— | **未变** ✅ |
| 两个资产（含大小） | 146,836,356 B ／ 272,616,859 B | **未变** ✅ |

⚠️ **注意删 tag 时 GitHub 不会删 release** —— 上面就是实测结果：release 和资产都活着，
重建同名 tag 后**自动重新关联**，不需要去页面重新选 tag。
（⚠️ 反过来说，如果**只在页面上重输 tag 名**，就会再次触发那个默认指向分支的解析。）

⚠️ **现在有两个 tag 指向同一个提交**：`v0.1.0-beta.1`（release 用的）与 `v0.1.0-beta1`（遗留，无 release 引用）。
无害 —— 两个都指向**正确的** `332192e`。想清掉遗留那个：
```bash
git push origin :refs/tags/v0.1.0-beta1
```
⚠️ 但**留着也不影响任何东西**，且删掉会让"曾经的拼写"无处可查 ⇒ 🔶 **倾向留着**。
