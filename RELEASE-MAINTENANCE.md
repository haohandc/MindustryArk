...
finding and cited as one. It reached the two READMEs, `RELEASE.md` (both halves)
...
#### Closed, by playing it (2026-09-23)

⭐ **A match has been played.** The user ran all three and reported them working: LAN play,
searching public servers, and hosting locally.

The section used to end by keeping one thing open on purpose -- "everything above says the
pieces are present and reachable, which is a different claim from 'joining a server works'".
That distinction was the right one to draw, and it is now closed by the only thing that could
close it: actually joining rather than measuring the platform underneath.


### 2.15 The README pair was swapped, so the landing page is Chinese

**2026-09-23.** The release body had just been made Chinese-only, and the user extended the
same reasoning to the repository's own front page: `README.md` was English while
`README.zh-CN.md` was Chinese, so a visitor landed on English while every other user-facing
document -- release body, `RELEASE.md`, `docs/` -- led with Chinese.

```
before                          after
README.md        (English)  ->  README.md        (Chinese)
README.zh-CN.md  (Chinese)  ->  README.en.md     (English)
```

`README.en.md` rather than `README.en-US.md`: this repo already spells its English documents
`release-notes/<version>.en.md` (see the release-body section), and one convention beats two.

⚠️ **A rename alone breaks seven files.** The pair is cross-linked in two ways: each README's
first line switches language, and seven other documents open with a pair of "back to README"
links -- one Chinese, one English, pointing at *different files*.

```bash
git grep -n 'README\.zh-CN\.md\|README\.md'
```

⇒ all seven had to move with it. The English release notes were the subtle one: they ended with
the `#readme` anchor, and **`#readme` resolves to whatever `README.md` is** -- now Chinese -- so
an English page would have sent its reader to a document they cannot read. It names
`README.en.md` outright now.

⚠️ **Replace whole lines, not tokens.** The obvious `README.md -> README.en.md` substitution also
matches `payload-src/README.md`, a different file entirely; and because every affected line
contains *both* filenames, a two-step replacement would rewrite its own output. Matching complete
link lines can do neither.
...
`RELEASE.md` — do not delete it from those to "avoid duplication"; they are the manual.
