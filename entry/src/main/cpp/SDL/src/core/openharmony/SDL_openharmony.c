...
/*
 * Attach SDL's XComponent callbacks to a component.
 *
 * TWO MAPPINGS OF THIS LIBRARY EXIST IN THE PROCESS, and this is what reconciles
 * them. The ArkTS XComponent loads one, and the callbacks it registers with land
 * there; the game loads the other, and that is the one that creates the
 * SDL_Window and owns the event queue.
 *
 * Input cannot simply be handed from one to the other. The XComponent's mapping
 * has no window of its own -- dereferencing it faults at offset 0x20, which is
 * SDL_Window.w, and that is exactly what happened the first time a control was
 * tapped. Handing it the other mapping's SDL_Window does not work either: sending
 * an event through it uses the other copy's event queue while running this copy's
 * code, and that corrupts memory. Both were tried.
 *
 * What does work is to move the callbacks themselves, so the mapping that owns
 * the window is also the one that receives the events. Registering again replaces
 * the previous set, so this is safe to call after the initial registration.
 */
static void register_xcomponent_callbacks(OH_NativeXComponent *nativeXComponent)
{
...
/*
 * Take the XComponent's callbacks over, called by the window code of whichever
 * mapping actually creates the window. See register_xcomponent_callbacks.
 */
void SDL_OpenHarmonyAdoptXComponent(void *component)
{
    if (component) {
        register_xcomponent_callbacks((OH_NativeXComponent *) component);
    }
}
