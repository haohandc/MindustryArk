...
/*
 * The SDL_Window to report input against, or NULL when this mapping of the
 * library has none -- in which case the caller must drop the event.
 *
 * There are two mappings of libSDL3.so in this process. The one the ArkTS
 * XComponent loads receives the input callbacks but never creates a window; the
 * other one creates the window and owns the event queue. OPENHARMONY_Window is
 * therefore NULL here for as long as the callbacks still belong to the first
 * mapping, and the first thing the touch path does with NULL is read ->w, which
 * faults at offset 0x20 -- SDL_Window.w -- which is the crash this guards.
 *
 * Reading the other mapping's window out of the bridge would make that
 * dereference succeed and be worse, not better: SDL_SendTouch would then run this
 * mapping's code against the other mapping's event queue, and the two do not
 * agree on any of the structures involved. That was tried and it aborts.
 *
 * The real fix is elsewhere: when the window is created, that mapping takes the
 * callbacks over (SDL_OpenHarmonyAdoptXComponent), and from then on NULL never
 * happens here. Until that moment has passed, events are dropped, which is the
 * correct thing to do with input for a window that does not exist yet.
 */
static SDL_Window *openharmony_window(void)
{
 ...
 *
 * It exists because the platform's contact count was the open question behind
 * "pinch does nothing": SDL is known to hand out one finger per contact, and the
 * Java input layer used to ignore finger events entirely, so the only way to
 * tell "the OS reports one contact" apart from "we drop the second" was to look
 * at what arrives here. Measured 2026-09-20: npoints reaches 2, with two
 * distinct ids -- so the loss was further up, and pinch was fixed in Arc's
 * SdlInput instead.
 *
 * It reports ONLY multi-contact events. Logging every event would mean an
 * fopen/fclose pair per touch event, and dragging a finger generates hundreds
 * per second -- file I/O on the input path of a game is a stutter waiting to
 * happen. Single-finger traffic (taps, drags, the overwhelming majority) costs
 * nothing; only a genuine pinch writes anything, which is exactly when someone
 * would want to look.
 *
 * Append-only on purpose: this file cannot be truncated from the host (the
 * sandbox is readable but not writable over hdc), so read it as an offset-based
 * delta or a previous run's fingers will be mistaken for this run's.
 *
 * The hint is recorded alongside because turning touch-to-mouse emulation off is
 * the other half of the pinch fix. If it silently did not take, every touch
 * would arrive twice -- once as an emulated mouse and once as a finger -- which
 * looks like a working touchscreen with inexplicable double-activations rather
 * than like a failed setting. Reading it back from the copy that actually
 * dispatches these events is the only way to tell those apart.
 */
#define OHOS_TOUCH_LOG "/data/storage/el2/base/files/sdl_touch.log"

/*
 * Reads: type, id, contact count, the emulation hint, then every contact.
 *
 * NOTE: keep this text plain. An earlier revision of this very comment contained
 * the typo "pin()ch" and, separately, the fprintf below once held the same typo
 * inside the string literal -- which would have gone straight into the log. No
 * harm done either time, but a probe whose own output is wrong is worth less
 * than no probe, and this file is the one place where the *text* is the data.
 */
static void touch_probe_dump(const OH_NativeXComponent_TouchEvent *e)
{
    if (e->numPoints < 2) {
        return;  /* single contact: the common case, and not what this is for */
    }

    FILE *f = fopen(OHOS_TOUCH_LOG, "a");
    if (!f) {
        return;
    }
    ...
    int off = 0;
    pts[0] = '\0';
    for (int32_t i = 0; i < e->numPoints && i < 10; i++) {
        int n = SDL_snprintf(pts + off, sizeof(pts) - (size_t) off,
                             "%s[%d id=%d xy=%d,%d type=%d]",
                             (i > 0) ? " " : "", (int) i,
                             (int) e->touchPoints[i].id,
                             (int) e->touchPoints[i].x, (int) e->touchPoints[i].y,
                             (int) e->touchPoints[i].type);
        if (n <= 0) {
            break;
        }
        off += n;
        if ((size_t) off >= sizeof(pts) - 1) {
            break;
        }
    }

    const char *touchMouse = SDL_GetHint(SDL_HINT_TOUCH_MOUSE_EVENTS);
    fprintf(f, "TOUCH type=%d id=%d npoints=%d touchMouseEvents=%s pts=%s\n",
            (int) e->type, (int) e->id, (int) e->numPoints,
            touchMouse ? touchMouse : "(unset)", pts);
    fclose(f);
}
...
{
    /* No window in this mapping means no event queue to put an event on, and the
     * callbacks have not been taken over yet -- see openharmony_window(). Drop
     * the event rather than dereference it. */
...
{
    /* see the note in SDL_OpenHarmonyDispatchTouchEvent */
    if (!openharmony_window()) {
        return;
    }
    OH_NativeXComponent *xcomponent = (OH_NativeXComponent *) component;
