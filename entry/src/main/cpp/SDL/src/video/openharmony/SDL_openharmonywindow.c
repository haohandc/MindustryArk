...
/* Takes the XComponent's callbacks over for this mapping of the library. */
extern void SDL_OpenHarmonyAdoptXComponent(void *component);
...
bool OPENHARMONY_CreateWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    /*
     * Take the XComponent's callbacks over.
     *
     * This is the mapping that owns the window and the event queue, so it has to
     * be the mapping that receives input. Until this point the callbacks belong
     * to the other mapping -- the one the ArkTS XComponent loaded -- where this
     * window does not exist and every touch dereferences NULL.
     *
     * The component pointer comes from the bridge because the SurfaceCreated
     * callback that carries it also ran in the other mapping. Done once: there is
     * one window and one component.
     */
    {
        static bool adopted = false;
        if (!adopted) {
            void *comp = SDL_OpenHarmonyBridgeGet(SDL_OPENHARMONY_BRIDGE_COMPONENT);
            if (comp) {
                adopted = true;
                SDL_OpenHarmonyAdoptXComponent(comp);
            }
        }
    }
